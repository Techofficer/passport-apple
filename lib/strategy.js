...
OAuth2Strategy.prototype.authenticate = function(req, options) {
	...
	if (req.query && req.query.code){
		...
		let params = {clientID: self._options.clientID, redirectUri: redirectUri, clientSecret: appleSignin.getClientSecret(self._options)};
		appleSignin.getAuthorizationToken(req.query.code, params).then(token => {
        	...
        	appleSignin.verifyIdToken(idToken).then(jwtClaims => {
        		const profile = {};
